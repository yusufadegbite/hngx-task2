...
const FeaturedMovie = () => {
  ...
  const getMovie = () => {
    fetch("api_key=cda4ffa2942badacf7d635acb2bbe728")
      .then((res) => res.json())
  ...
  return (
    <div className="container px-[4rem] md:px-[10rem] p-5 flex flex-col justify-center space-y-3">
      <div className="flex w-full justify-between pt-1">
        <div>
          <h2 className="text-[22px] font-medium">Featured Movie</h2>
        </div>
        <div className="flex">
          <p className="text-red-700 font-semibold">See more</p>
          <img
            className="w-[2rem] h-[1.5rem] mt-[0.09rem]"
            src={rightIcon}
            alt=""
          />
        </div>
      ...
      <div
        ...
        className="relative grid grid-cols-1 md:grid-cols-4 sm:grid-cols-2 md:gap-4 gap-2"
      >
        {movieList &&
          movieList.map((movie) => {
            return (
                ...
                <div className="">
                  ...
                  >
                    {favoriteStatus[movie.id] ? (
                      <img className="absolute m-2" src={favoriteRed} alt="" />
                    ) : (
                      <img className="absolute m-2" src={favorite} alt="" />
                    )}
                  </div>
                  <Link to={`/movie/${movie.id}`}>
                    ...
                    <p data-testid="movie-poster" className="">
                      {movie.release_date}
                    </p>
                  </Link>
                </div>
